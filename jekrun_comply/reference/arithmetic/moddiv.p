...
runner:case(truncate, -2, arithmetic_moddiv, 'ISO 9.1.7, XLOG 2') :-
   -2.251799813685248E15 is truncate(- 2^51-1/2).
...
runner:case(floor, -2, arithmetic_moddiv, 'ISO 9.1.7, ISO 2') :-
   -1.0 is floor(-0.4).
runner:case(floor, -2, arithmetic_moddiv, 'ISO 9.1.7, XLOG 1') :-
   2.251799813685248E15 is floor(2^51+1/2).
...
runner:case(ceiling, -2, arithmetic_moddiv, 'ISO 9.1.7, XLOG 2') :-
   8.0 is ceiling(7.6).
runner:case(ceiling, -2, arithmetic_moddiv, 'ISO 9.1.7, XLOG 3') :-
   2.251799813685249E15 is ceiling(2^51+1/2).
...
runner:case(round, -2, arithmetic_moddiv, 'ISO 9.1.7, XLOG 1') :-
   -2.251799813685249E15 is round(- 2^51-1/2).
...
runner:case(//, -3, arithmetic_moddiv, 'ISO 9.1.7, ISO 23') :-
   10 is 140//(3+11).
runner:case(//, -3, arithmetic_moddiv, 'ISO 9.1.7, XLOG 1') :-
   -2 is -5//2.
runner:case(//, -3, arithmetic_moddiv, 'ISO 9.1.7, XLOG 2') :-
...
runner:case(rem, -3, arithmetic_moddiv, 'ISO 9.1.7, XLOG 7') :-
   ...
   E == evaluation_error(zero_divisor).
/* extra */
runner:case(rem, -3, arithmetic_moddiv, 'ISO 9.1.7, XLOG 8') :-
   -0.5 is -7.5 rem 3.5.
...
runner:case(div, -3, arithmetic_moddiv, 'Corr.2 9.1.3, XLOG 5') :-
   ...
   E == type_error(evaluable, foobar/0).
/* extra */
runner:case(div, -3, arithmetic_moddiv, 'Corr.2 9.1.3, XLOG 6') :-
   -3 is -7.5 div 3.5.
...
runner:case(mod, -3, arithmetic_moddiv, 'ISO 9.1.7, ISO 36') :-
   ...
   E == evaluation_error(zero_divisor).
/* extra */
runner:case(mod, -3, arithmetic_moddiv, 'ISO 9.1.7, XLOG 3') :-
   3.0 is -7.5 mod 3.5.
