...
% freeze(+Term, +Goal)
runner:ref(freeze, 2, term_delay, 'Term 1.0.0, 1.6').
runner:case(freeze, 2, term_delay, 'Term 1.0.0, 1.6, XLOG 1') :-
   call_residue((freeze(X, X > 0), freeze(X, X < 0)), L),
   L == [freeze(X, X > 0), freeze(X, X < 0)].
runner:case(freeze, 2, term_delay, 'Term 1.0.0, 1.6, XLOG 2') :-
   freeze(X, X > 0), freeze(X, X < 0), \+ X = 1, \+ X = -1.
runner:case(freeze, 2, term_delay, 'Term 1.0.0, 1.6, XLOG 3') :-
   freeze(X, X > Y), Y = 0, \+ X = -1.
runner:case(freeze, 2, term_delay, 'Term 1.0.0, 1.6, XLOG 4') :-
   freeze(X, (X = f(a); X = f(b))), \+ X = g(_).
runner:case(freeze, 2, term_delay, 'Term 1.0.0, 1.6, XLOG 5') :-
   findall(X, (freeze(X, (X = f(a); X = f(b))), X = f(_)), [Y|_]),
   Y == f(a).
runner:case(freeze, 2, term_delay, 'Term 1.0.0, 1.6, XLOG 6') :-
   findall(X, (freeze(X, (X = f(a); X = f(b))), X = f(_)), [_, Y|_]),
   Y == f(b).
runner:case(freeze, 2, term_delay, 'Term 1.0.0, 1.6, XLOG 7') :-
   findall(X, (freeze(X, (X = f(a); X = f(b))), X = f(_)), [_, _]).
runner:case(freeze, 2, term_delay, 'Term 1.0.0, 1.6, XLOG 8') :-
   freeze(X, 1), catch(X = 0, error(E, _), true),
   E = type_error(callable, 1).

% when(+Cond, +Goal)
runner:ref(when, 2, term_delay, 'Term 1.0.0, 1.7').
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 1') :-
   call_residue(when((nonvar(X), nonvar(Y)), X < Y), L),
   L == [when((nonvar(X), nonvar(Y)), X < Y)].
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 2') :-
   call_residue((when((nonvar(X), nonvar(Y)), X < Y), X = 0), L),
   L == [when(nonvar(Y), 0 < Y)].
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 3') :-
   when((nonvar(X), nonvar(Y)), X < Y), \+ (X = 0, Y = -1), \+ (X = 1, Y = 0).
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 4') :-
   when((nonvar(X); ground(Y)), atom_codes(X, Y)), X = abc,
   Y == "abc".
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 5') :-
   when((nonvar(X); ground(Y)), atom_codes(X, Y)), Y = "abc",
   X == abc.
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 6') :-
   findall(X, (when(nonvar(X), (X = f(a); X = f(b))), X = f(_)), [Y|_]),
   Y == f(a).
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 7') :-
   findall(X, (when(nonvar(X), (X = f(a); X = f(b))), X = f(_)), [_, Y|_]),
   Y == f(b).
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 8') :-
   findall(X, (when(nonvar(X), (X = f(a); X = f(b))), X = f(_)), [_, _]).
runner:case(when, 2, term_delay, 'Term 1.0.0, 1.7, XLOG 9') :-
