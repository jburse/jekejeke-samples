...
public final class Query {
    ...
    public Object makeQuery(TermVar[] vars) {
        ...
        if (!"".equals(name))
            literals.add(0, new TermCompound(inter, "=", name, vars[COLUMN_NAME]));
        if (!"".equals(agefrom))
            literals.add(new TermCompound(inter, "=<", Integer.valueOf(agefrom), vars[COLUMN_AGE]));
        if (!"".equals(ageto))
            literals.add(new TermCompound(inter, ">=", Integer.valueOf(ageto), vars[COLUMN_AGE]));
        Object queryTerm = literals.get(literals.size() - 1);
        for (int i = literals.size() - 2; i >= 0; i--)
            queryTerm = new TermCompound(inter, ",", literals.get(i), queryTerm);
        return queryTerm;
    }
    ...
    public void listRows(TermVar[] vars, Object queryTerm) {
        try {
            ...
            CallIn callin = getInter().iterator(queryTerm);
