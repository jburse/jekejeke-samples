...
inform(L) :- write('Question: '), inform1(L), nl.

inform1([]).
inform1([H|T]) :- out(H), put(" "), inform1(T).
...
show_result(Label, TimeA, TimeB) :-
   Time is TimeB-TimeA,
   write(Label), write(': '), write(Time), write(' ms'), nl.

process(Level, Time) :-
   uptime(Time1),
   (between(1, 150, _),
   ed(_, Sentence, _),
   process(Sentence, _, Level),
   fail; true),
   uptime(Time2),
   Time is Time2-Time1.

process(Sentence, Answer, Level) :-
   (Level > 0 -> sentence(E, Sentence, [], [], []), !; true),

   (Level > 1 -> i_sentence(E, QT),
   clausify(QT, UE),
   simplify(UE, S), !; true),

   (Level > 2 -> qplan(S, S1), !; true),

   (Level > 3 -> answer(S1, Answer), !; true).
