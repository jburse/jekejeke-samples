...
database(aggregate(X,Y,Z)) :-
   aggregate(X, Y, Z).
database(one_of(X,Y)) :-
   one_of(X, Y).
database(ratio(X,Y,Z)) :-
   ratio(X, Y, Z).
database(card(X,Y)) :-
   card(X, Y).
database(african(X)) :-
...
database(asian(X)) :-
   asian(X).
database(borders(X,Y)) :-
   borders(X, Y).
database(capital(X)) :-
   capital(X).
database(capital(X,Y)) :-
   capital(X, Y).
database(circle_of_latitude(X)) :-
   circle_of_latitude(X).
database(city(X)) :-
   city(X).
database(continent(X)) :-
   continent(X).
database(country(X)) :-
   country(X).
database(drains(X,Y)) :-
...
database(european(X)) :-
   european(X).
database(exceeds(X,Y)) :-
   exceeds(X, Y).
database(flows(X,Y)) :-
   flows(X, Y).
database(flows(X,Y,Z)) :-
   flows(X, Y, Z).
database(in(X,Y)) :-
   in(X, Y).
database(latitude(X)) :-
   latitude(X).
database(latitude(X,Y)) :-
   latitude(X, Y).
database(longitude(X)) :-
   longitude(X).
database(longitude(X,Y)) :-
   longitude(X, Y).
database(northof(X,Y)) :-
   northof(X, Y).
database(ocean(X)) :-
   ocean(X).
database(place(X)) :-
...
database(population(X)) :-
   population(X).
database(population(X,Y)) :-
   population(X, Y).
database(region(X)) :-
   region(X).
database(rises(X,Y)) :-
...
latitude(C, L--degrees) :-
   country(C, _, L, _, _, _, _, _, _, _).
longitude(C, L--degrees) :-
   country(C, _, _, L, _, _, _, _, _, _).
area(C, A--ksqmiles) :-
   country(C, _, _, _, A, _, _, _, _, _).
population(C, P--thousand) :-
   city(C, _, P).
population(C, P--million) :-
   country(C, _, _, _, _, _, P, _, _, _).
capital(C, Cap) :-
   country(C, _, _, _, _, _, _, _, Cap, _).
