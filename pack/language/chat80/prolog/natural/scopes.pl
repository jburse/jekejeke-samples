/**
 * Prolog text scopes from Chat80 as a module.
 *
 * Warranty & Liability
...
pre_apply(aggr(F,Value,L,Head,Pred), Det, X, P1, P2, Y, Quants,
   quant(Det,X,
      (S^(setof(Range:Domain,P,S),aggregate(F,S,Value)),P2),Y)) :-
   close_tree(Pred, R),
...
apply(proportion(_-V), _, X, P, Y, Q,
   S^(setof(X,P,S),N^(numberof(Y,(one_of(S,Y),Q),N),M^(card(S,M),ratio(N,M,V))))).
